      ...
      <div className="mx-auto flex w-full max-w-6xl flex-1 flex-col gap-8 px-4 py-8 sm:px-6 lg:py-12">
        ...
        <footer className="animate-fade-in flex flex-col items-center gap-3 pb-8 text-center text-sm text-slate-500">
          <div className="flex items-center gap-4">
            <span>
              Desarrollado por{" "}
              <span className="font-medium text-slate-400">MUYUQI</span>{" "}
              en{" "}
              <span className="font-medium text-slate-400">AUDIO-DOSIS</span>
              , La Paz — Bolivia
            </span>
            <span className="text-slate-700">·</span>
            <span className="text-xs text-slate-500">Si buscas alguna mejora</span>
            <a
          ...
          <a
            href="https://github.com/VictorJimenezM/GeneradorVideoWave"
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center gap-1.5 rounded-lg border border-slate-800 bg-slate-900/60 px-3 py-1.5 text-xs text-slate-400 transition-all duration-200 hover:border-slate-600/50 hover:text-slate-300"
          >
            <svg className="h-3.5 w-3.5" fill="currentColor" viewBox="0 0 24 24">
        ...
        <p className="pb-4 text-center text-xs text-slate-600">Version 1.10.4</p>
      </div>
    </div>
  );
}
