...
- **v1.13.1** — Fix: auto-rotate de imagen/filtro se detenía al cambiar el efecto SUB. El acumulador de beats (`bgLastBeatTimeRef`) se adelantaba a `currentTime` durante frames pesados (redrawFractalCanvas → syncAllCanvasSizes). Se añade clamp para que nunca supere el tiempo actual.
