...
| 6 | **Texto** (collapsible) | Input título + posición (10 presets de layout). Grupo **Serios**: fuente (8: 4 clásicas + 4 extremas Google Fonts), Negrita, Cursiva, alineación horizontal/vertical, Tamaño. Grupo **Creativos**: Texto en curva, Movimiento (Ninguno/Pulso/Flotar/Zoom/Rotación) + Intensidad. Color picker |
...
- **Layout preset** (`titlePreset`, 10 posiciones): al elegirlo, `applyTitlePreset(id)` setea align/valign/tamaño desde el preset y conserva `x/y` (posición); los controles de fuente/peso/estilo/curva/movimiento son independientes.
- **Controles creativos**: slider **Texto en curva** (−1…1, arco) · `<select>` **Movimiento** (`none`/`pulse`/`float`/`zoom`/`rotate`) · slider **Intensidad** (0–1).
